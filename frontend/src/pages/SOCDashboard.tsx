...
export default function SOCDashboard() {
  ...
  // Simulated diagnostic logs based on user feedback (resolves HTTP 404 and Warning visibility)
  const systemLogs = [
    { type: 'warning', source: 'HuggingFace Hub', message: '404 Not Found: additional_chat_templates for bert-finetuned-phishing', time: '09:32:01' },
    { type: 'critical', source: 'Tool: Whois', message: "ModuleNotFoundError: No module named 'whois'", time: '09:31:45' },
    { type: 'warning', source: 'PII Scrubber', message: 'Masked 1 instance of sensitive email data', time: '09:31:30' },
    { type: 'info', source: 'Model Loader', message: 'Bert embeddings.position_ids loaded with unexpected weights', time: '09:31:10' },
    { type: 'warning', source: 'HF Auth', message: 'Sending unauthenticated requests to HF Hub. Set HF_TOKEN.', time: '09:30:55' },
  ];
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen font-display">
      ...
      <main className="max-w-[1440px] mx-auto p-6 space-y-6">
        {/* Tab Selection Section - Added without deleting existing content */}
        <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-800 w-fit rounded-lg border border-slate-200 dark:border-slate-700">
          <button 
            onClick={() => setActiveTab('overview')}
            className={`px-6 py-1.5 text-xs font-bold uppercase tracking-widest rounded-md transition-all ${activeTab === 'overview' ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Overview
          </button>
          <button 
            onClick={() => setActiveTab('health')}
            className={`px-6 py-1.5 text-xs font-bold uppercase tracking-widest rounded-md transition-all flex items-center gap-2 ${activeTab === 'health' ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            System Health
            {systemLogs.length > 0 && <span className="size-2 bg-amber-500 rounded-full animate-pulse"></span>}
          </button>
        </div>
        ...
        {activeTab === 'overview' ? (
                        ...
                        <div className="flex flex-col gap-4 mt-6">
                          <div className="flex gap-4 relative group">
                            ...
                            <div className="flex-1 bg-primary/10 border border-primary/20 p-4 rounded-lg ring-1 ring-primary/30 shadow-sm shadow-primary/10">
                              ...
                              <div className="prose prose-sm dark:prose-invert max-w-none prose-p:leading-relaxed prose-strong:text-primary prose-ul:list-disc prose-li:marker:text-primary">
          ...
          /* SYSTEM HEALTH VIEW - Surfacing Diagnostics Integrated with Layout */
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in pb-12">
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
                <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
                  <h4 className="font-bold text-sm uppercase tracking-tight">Diagnostic Observation Stream</h4>
                  <span className="text-[10px] font-bold text-slate-400">Live Integration Logs</span>
                </div>
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {systemLogs.map((log, i) => (
                    <div key={i} className="p-4 flex gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <div className={`mt-1 size-8 rounded flex items-center justify-center shrink-0 ${log.type === 'critical' ? 'bg-red-100 text-red-600' : log.type === 'warning' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'}`}>
                        <span className="material-symbols-outlined text-lg">
                          {log.type === 'critical' ? 'dns' : log.type === 'warning' ? 'notification_important' : 'info'}
                        </span>
                      </div>
                      <div className="flex-1">
                        <div className="flex justify-between items-center mb-1">
                          <span className={`text-[10px] font-black uppercase tracking-widest ${log.type === 'critical' ? 'text-red-500' : log.type === 'warning' ? 'text-amber-500' : 'text-blue-500'}`}>{log.source}</span>
                          <span className="text-[10px] text-slate-400 font-bold">{log.time}</span>
                        </div>
                        <p className="text-xs font-bold text-slate-800 dark:text-slate-200">{log.message}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-6">
               <div className="bg-slate-900 rounded-xl p-6 text-white shadow-xl">
                 <h4 className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-6 border-b border-white/5 pb-4">Infrastructure Status</h4>
                 <div className="space-y-4">
                    <div className="flex justify-between text-xs font-bold">
                       <span>Models / Transformers</span>
                       <span className="text-emerald-400">Stable</span>
                    </div>
                    <div className="flex justify-between text-xs font-bold">
                       <span>HF Connectivity</span>
                       <span className="text-amber-500">Unauthenticated</span>
                    </div>
                    <div className="flex justify-between text-xs font-bold">
                       <span>WHOIS Module</span>
                       <span className="text-red-500">Patching...</span>
                    </div>
                 </div>
               </div>
            </div>
      ...
      {/* Side Panels - Shared by all tabs */}
      {showIntel && (
